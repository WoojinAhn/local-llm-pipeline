...
한국어 질문
    │
    ▼
[Qwen3-14B] ── 한→영 번역 + 검색 필요 여부 판별 (SEARCH:yes/no)
    │
    ├── SEARCH:yes ──▶ [Brave 한국어 + Tavily 영어] 병렬 검색
    │                          │
    │                          ▼
    │                   검색 결과 컨텍스트
    │                          │
    ▼                          ▼
[DeepSeek R1 70B] ── 영어 분석 (S급) + 검색 컨텍스트 (있을 경우)
    │
    ▼
...
Korean input
    │
    ▼
[Qwen3-14B] ── KR→EN translation + search judgment (SEARCH:yes/no)
    │
    ├── SEARCH:yes ──▶ [Brave Korean + Tavily English] parallel search
    │                          │
    │                          ▼
    │                   Search result context
    │                          │
    ▼                          ▼
[DeepSeek R1 70B] ── English analysis (S-tier) + search context (if available)
    │
    ▼
