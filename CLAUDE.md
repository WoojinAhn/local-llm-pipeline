...
- Compute/render split (#71): `pipeline_events()` is a generator of `(kind, payload)` events and touches no terminal; `pipeline()` drives it and returns a `PipelineResult`; `ConsoleRenderer` + `render_result()` produce the output. A caller that only wants the answer calls `pipeline(query)` with no `on_event`. The streaming helpers are generators (`_qwen_chunks`, `_reasoner_chunks`, `_translate_chunks`, `_analyze_chunks`) with thin `_drain`-based drivers keeping the old `--reasoner-only`/`--qwen-only`/`--translate-only` behaviour.
...
- **Do not add responsibilities to `TRANSLATE_KO_TO_EN`**: it already does double duty (translate + `SEARCH:yes/no`), and a third judgment degrades the first two. Adding `LOCAL:yes/no` made Qwen answer questions instead of translating them (`설명해줘` → an explanation, 3/3 runs) and treat SEARCH/LOCAL as mutually exclusive, so weather queries came back `SEARCH:no` and skipped search entirely. Both were measured A/B against the unmodified prompt in #67; the fix was a separate one-line judge call (`judge_location()`), not more prompt tuning. New per-query judgments belong in their own call.
...
When judging whether a backlog item is worth doing, cost two things separately:

- **Spike cost** — what it takes to answer the open question (a download, one A/B, an hour).
- **Adoption cost** — what it takes to make it the default (integration, regression runs, docs).

Collapsing them kills cheap, reversible experiments on the grounds of expensive rollouts. That
mistake is on record: #35 and #39 were briefly judged redundant against #40 by pricing them at
adoption cost, when both are low-cost spikes against the *default* reasoner path — which #40
does not change, since it lands flag-gated.
...
- Stream stage 3/4 instead of buffering behind a spinner — #64. #71 removed the structural blocker: `pipeline_events()` emits `("token", (stage, text))` for every generating stage, and `ConsoleRenderer` drops them. What remains is the choice #71 deliberately did not make — raw tokens and the final `Markdown()` re-render want the same screen region, so #64 must pick one of stream-then-rerender / clear-and-redraw / plain-text-while-streaming.
- Result-to-file save option — #65. #71 landed the return value (`PipelineResult`, a dataclass, so `dataclasses.asdict()` serializes it) including `sources` URLs, the search queries as issued, and per-stage timings. What is left is the file format and the flag.
- Textual TUI — #8. Scoped to the TUI alone; #64/#65 were split out of it. Its "markdown streaming rendering" is the consumer side (widget updates) and is distinct from #64, the producer side.
